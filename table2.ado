...
program define table2
	...
	syntax [if] [in] [fweight], ///
		...
		[plusminus]			/// report contn vars as mean Ѡsd instead of mean (sd)
			...
			* continuous, normally distributed variable
			if "`vartype'"=="contn" {
				...
				* collapse to table1 format
				...
				if "`plusminus'"=="plusminus" gen factor="`varlab', mean ѠSD"
			...
			if "`vartype'"=="conts" {
				...
				* collapse to table1 format
				collapse (p50) p50=`varname' (p25) p25=`varname' ///
					(p75) p75=`varname' (count) n=`varname', by(`groupnum')
				qui gen median_iqr=string(p50, "`varformat'") + ///
					" (" + string(p25, "`varformat'") + ///
					", " + string(p75, "`varformat'") + ")"
				if "`cmissing'"=="cmissing" {
					replace median_iqr=median_iqr + " (n=" + string(n) + ")"
				}
			...
			* categorical variable
			if "`vartype'"=="cat" | "`vartype'"=="cate" {
				...
				* collapse to table1 format
				...
				if "`row'" != "" qui egen tot = total(_freq), by(`varnum') // Thêm lựa chọn để tính % theo hàng
				
				* default format is 0 decimal places if <100 cases, otherwise 1 dp
				* (for categorical variables, format is for % not the frequency)
				* however this default can be overridden by the cformat() option
				if "`varformat'"=="" {
					if "`cformat'"=="" {
						sum tot, meanonly
					...
					else local varformat `cformat'
				}

				* finish restructuring to table1 format
				qui gen perc=string(100*_freq/tot, "`varformat'")
				qui replace perc="~0" if _freq!=0 & real(perc)==0 // chỉnh lại label
				
				if "`percent'"=="percent" qui gen n_perc=perc + "%"
				else qui gen n_perc=string(_freq) + " (" + perc + "%)"
				
				// drop _freq tot perc 
				drop tot n_perc // không lấy n_perc nữa bởi vì chia làm 2 hàng
				qui la var perc "Tỷ lệ (%)"
				// qui reshape wide n_perc, i(`varnum') j(`groupnum')
				qui reshape wide _freq perc, i(`varnum') j(`groupnum') //chỉnh lại vì không còn dữ n_perc
				
				// rename n_perc* `groupnum'*
				qui tostring _freq*, replace
				rename _freq* `groupnum'* // biến _freq thì phải đổi về dạng string để ghép được
			...
			* binary variable
			if "`vartype'"=="bin" | "`vartype'"=="bine" {
				...
				* finish restructuring to table1 format
				...
				drop tot n_perc //đã thay đổi từ code cũ, chỉ xoá phần tổng với phần n
				qui tostring _freq, replace
				qui reshape wide _freq perc, i(`varname') j(`groupnum')
				qui drop `varname'
				qui gen factor="`varlab'" if _n==1
				qui clonevar factor_sep=factor
				rename n_perc* `groupnum'*
	...
	* label other variables
	lab var factor "Đặc điểm"
	capture lab var level "Level"
	capture lab var test "Test thống kê"
	if `groupcount'==1 lab var `groupnum'1 "Giá trị"
	...
	* format p-values
	if `groupcount'>1 {
		...
		lab var pvalue "Giá trị p"
	}

	* create a row containing variable labels - for nicer output
	qui count
	local newN=r(N) + 1
	local newN2=r(N) + 2 // thêm hàng cho tần số và tỷ lệ
	qui set obs `newN2'
	qui desc, varlist
	foreach var of varlist `r(varlist)' {
		capture replace `var'="`: var lab `var''" in `newN2' // thay bằng newN2
	}
	qui replace sort1=0.5 in `newN' // thay bằng 0.5
	qui replace sort1=0   in `newN2'
	* thay đổi giá trị của biến tỷ lệ phần trăm
	cap d perc*
	if !_rc {
		foreach v of varlist perc* {
			qui replace `v' = "Tỷ lệ (%)" in `newN'
			qui replace `v' = "" in `newN2'
			qui replace `v' = "100" in `=`newN'-1' // thêm lựa chọn
		}
	}
	foreach v of varlist `groupnum'* {
		qui replace `v' = "Tần số" in `newN'
	}
	* clean up variables in preparation for display
	// order factor `groupnum'*
	* order lại bởi vì có nếu có thêm biến perc
	order factor, first
	cap d perc*
	if !_rc {
		foreach v of local levels {
			qui order perc`v', after(`groupnum'`v')
		}
	}
	cap d perc*
	if _rc!=0 {
