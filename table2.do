...
program define table2
			...
			* categorical variable
			if "`vartype'"=="cat" | "`vartype'"=="cate" {
				...
				* finish restructuring to table1 format
				...
				drop tot n_perc
				qui reshape wide _freq perc, i(`varnum') j(`groupnum')
				rename perc* `groupnum'*
	...
	* clean up variables in preparation for display
	order factor _freq* `groupnum'*
	cap order `groupnum'0, a(_freq0)
	cap order _freq1, a(`groupnum'0)
	cap order `groupnum'1, a(_freq1)
	cap order  _freq2, a(`groupnum'1)
	cap order `groupnum'2, a(_freq2)
	...
	* if -saving- was specified then we'll save the table as an Excel spreadsheet
	if `"`saving'"'!="" export excel using `saving'.xlsx, `replace'
