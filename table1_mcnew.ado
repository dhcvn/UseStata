...
capture program drop table2_mc
program define table2_mc, sclass
...
local gmeanSD : display "geometric mean"`gsdleft'"geometric SD"`gsdright'
	...
	* if -saving- was specified then we'll save the table as an Excel spreadsheet
	...
	if `"`saving'"'!="" & `groupcount'==1 & "`chiacot'"=="" export excel factor-Total using  `saving'  // mc removed lonely , `replace'
	if `"`saving'"'!="" & `groupcount'>1  & "`chiacot'"=="" export excel factor-pvalue using  `saving'
	if `"`saving'"'!="" & `groupcount'==1 & "`chiacot'"!="" {
	    qui replace _columna_1 = "Tần số"  if factor == " "
		qui replace _columnb_1 = "Tỷ lệ %" if factor == " "
		qui replace _columna_1 = Total     if _columna_1 == ""
		ren _columna_1 tanso
		ren _columnb_1 phantram
		qui replace phantram = subinstr(phantram, "(", "", .)
		qui replace phantram = subinstr(phantram, ")", "", .)
		order factor, b(tanso)
        cap order level, a(factor)
		export excel factor-phantram using `saving'
	}
    if `"`saving'"'!="" & `groupcount'>1 & "`chiacot'"!="" {
		// foreach v of varlist _columna* {
		// 	qui replace `v' = "Tần số" if factor ==" "
		// }
		foreach v of varlist _columnb* {
			qui replace `v' = ""  if factor ==" "
			qui replace `v' = subinstr(`v', "(", "", .)
			qui replace `v' = subinstr(`v', ")", "", .)
		}
		local first: word 1 of `levels'
		ren _columna* `by'sepn*
		ren _columnb* `by'sepp*
		foreach l of local levels {
			cap replace `by'sepn_`l' = `by'_`l' in 2
		}
		order factor, b(`by'sepn_`first')
		cap order level, a(factor)
		order pvalue, last 
        export excel factor-pvalue using `saving'
	}
